...
- `src/pages/admin/` — Admin-Cockpit (Overview mit Nächster-Zug, KI/Denklogik, Trends, Posting, Zahlungen, Aktionen-Log, Archiv)
...
- Wichtige Edge Functions: `create-checkout` (Stripe, KEIN automatic_payment_methods-Parameter!), `stripe-connect` (Designer-Auszahlungskonten, s.u.), `stripe-webhook`, `generate-broll` (fal i2v), `generate-tryon` (Kolors Try-On, Model-Pool), `generate-product-shot`, `poll-broll` (schreibt fertige Clips in `video_assets`), `pawn-chat`, `studio-ai`, `pawn-actions` (Aktionsschicht mit Undo), `compute-trends`, `pawn-jarvis` (interne KI-Instanz, s.u.)
- Datenmodell-Kern: designers (house_number, plan haus/atelier/maison, brand_dna, stripe_account_id/stripe_charges_enabled/stripe_details_submitted, media_rights_granted_at), products (product_dna jsonb, designer_note, Maße), orders (application_fee_cents, destination_account — Buchhaltungs-Spur für Connect), campaigns, posting_queue, generation_requests, product_shot_requests, video_assets (Video-Archiv, s.u.), fashion_ontology (lernend, `learned`-Flag), user_memory, ai_actions_log, acquisition_leads (Akquise-Pipeline), jarvis_runs, jarvis_reports, site_content, ai_config
...
## Pläne & KI-Budget (Teil 6b)
Neue Staffel, Zahlen live aus `ai_config.plan_limits`/`plan_prices` (nichts hart verdrahtet): Haus 0€ (3 Videos Editorial-Regie mit PAWN-Emblem, 1 Signatur-Kostprobe, 5 Try-Ons/Produkt-Shots), Atelier 24€ (15 Videos davon 8 kinematisch ohne Emblem, 3 Signaturen, 25 Try-Ons+Produkt-Shots), Maison 99€ (40 Videos alle kinematisch, alle Signaturen + Wunsch-Signatur, unbegrenzte Try-Ons/Shots). Bestehende Abos behalten ihren alten Stripe-Preis — nur `eur_month` (Anzeige) wurde angehoben, `stripe_price_id` bleibt unangetastet. Internes KI-Budget je Haus/Monat (`ai_config.ai_budget_limits`, `ai_budget_ledger`-Tabelle, DB-Funktion `book_ai_spend`) bucht die Ist-Kosten jeder Generierung in Cent — informativ, die sichtbaren Kontingente bleiben das durchsetzende Limit. `generate-tryon`/`generate-product-shot` blocken jetzt freundlich, wenn das Monatskontingent aufgebraucht ist. Funnel „Welches Haus bist du?" (`PlanFunnel.tsx`) auf Landing (für Bewerber) und `/studio/plan` (für Designer, mit optionaler KI-Begründung aus der Brand-DNA).
