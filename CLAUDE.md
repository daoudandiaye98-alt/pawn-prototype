...
## Pläne & KI-Budget (Teil 6b, Preisumbau Teil 38 AP7, zurückgenommen Teil 38 WP7)
Es gibt wieder drei kaufbare Pläne: **Haus** (DB-Wert `haus`, 0€), **Atelier** (DB-Wert `atelier`) und **Maison** (DB-Wert `maison`) — jeder mit eigenem Preis live aus `ai_config.plan_prices.<plan>.eur_month`/`stripe_price_id`. Die Teil-38-AP7-Vereinfachung (nur noch "Frei" vs. "Paid", Atelier als auslaufendes Bestandsabo) ist auf Wunsch zurückgenommen: Atelier ist wieder ein regulärer, neu abschließbarer Plan mit eigener Preiskarte unter `/studio/plan` (`src/lib/planGate.ts` / `supabase/functions/_shared/planGate.ts`, `isPaidPlan` prüft nur noch "ist irgendein Bezahlplan", nicht mehr welcher). `create-checkout` akzeptiert neue Subscription-Anfragen auf `atelier` oder `maison`. Zahlen (Videos/Shots/Signaturen je Plan) bleiben live aus `ai_config.plan_limits`/`plan_prices` (nichts hart verdrahtet) — Haus: 3 Videos Editorial-Regie mit PAWN-Emblem, 1 Signatur-Kostprobe, 5 Try-Ons/Produkt-Shots; Atelier: 15 Videos (8 kinematisch), 3 Signaturen, 25 Try-Ons/Shots; Maison: 40 Videos alle kinematisch, alle Signaturen + Wunsch-Signatur, unbegrenzte Try-Ons/Shots. Ein Wechsel zwischen zwei laufenden Bezahlplänen (z. B. Atelier → Maison) läuft nie automatisch über einen zweiten Checkout, sondern als begleitete Anfrage ans Team (`message_threads`) — Gefahr sonst: zwei parallel laufende Abos. Internes KI-Budget je Haus/Monat (`ai_config.ai_budget_limits`, `ai_budget_ledger`-Tabelle, DB-Funktion `book_ai_spend`) bucht die Ist-Kosten jeder Generierung in Cent — informativ, die sichtbaren Kontingente bleiben das durchsetzende Limit. `generate-tryon`/`generate-product-shot` blocken jetzt freundlich, wenn das Monatskontingent aufgebraucht ist.
