...
## Pläne & KI-Budget (Teil 6b, Preisumbau Teil 38 AP7)
Nach außen gibt es seit Teil 38 AP7 nur noch zwei Pläne: **Frei** (DB-Wert `haus`, 0€) und **Paid** (DB-Wert `maison`, Preis live aus `ai_config.plan_prices.maison`). `atelier` bleibt als dritter DB-Wert bestehen, wird aber nicht mehr neu vergeben — bestehende Atelier-Häuser zahlen unverändert ihren alten Preis weiter und laufen technisch als "Paid" (`src/lib/planGate.ts` / `supabase/functions/_shared/planGate.ts`, `isPaidPlan`). `create-checkout` lehnt neue Subscription-Anfragen ab, die nicht auf den Paid-Schlüssel `maison` zielen. Zahlen (Videos/Shots/Signaturen je Plan) bleiben live aus `ai_config.plan_limits`/`plan_prices` (nichts hart verdrahtet) — Frei: 3 Videos Editorial-Regie mit PAWN-Emblem, 1 Signatur-Kostprobe, 5 Try-Ons/Produkt-Shots; Paid (`maison`): 40 Videos alle kinematisch, alle Signaturen + Wunsch-Signatur, unbegrenzte Try-Ons/Shots. **Offen für Daouda:** den neuen Paid-Preis in Stripe anlegen (die alten Atelier-/Maison-Preise dabei nur archivieren, nicht löschen) und die neue `stripe_price_id` direkt in `ai_config.plan_prices.maison` eintragen — sobald das passiert ist, greift der Checkout automatisch, ohne weiteren Code. Internes KI-Budget je Haus/Monat (`ai_config.ai_budget_limits`, `ai_budget_ledger`-Tabelle, DB-Funktion `book_ai_spend`) bucht die Ist-Kosten jeder Generierung in Cent — informativ, die sichtbaren Kontingente bleiben das durchsetzende Limit. `generate-tryon`/`generate-product-shot` blocken jetzt freundlich, wenn das Monatskontingent aufgebraucht ist. `/studio/plan` zeigt die zwei Karten Frei/Paid; ein Atelier-Bestandsabo sieht dort einen ehrlichen Hinweis statt einer eigenen dritten Karte.
