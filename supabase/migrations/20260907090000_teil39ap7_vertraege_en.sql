-- Teil 39 AP7 — englische Kulanz-Übersetzung der Designer-Verträge. Neue Spalte
-- body_markdown_en (nullable, deutsche Fassung bleibt in jedem Fall verbindlich —
-- siehe LegalTranslationNote-Muster). Die UPDATEs zielen auf die AP2-Vertragsversionen
-- (kind/version aus 20260902090000_teil39ap2_designer_vertraege.sql); falls jene
-- Migration zum Zeitpunkt dieses Laufs noch nicht angewendet wurde, treffen die UPDATEs
-- keine Zeile und bleiben folgenlos — sie greifen automatisch, sobald AP2 vorher lief
-- (Dateiname/Timestamp dieser Migration liegt bewusst nach AP2).
