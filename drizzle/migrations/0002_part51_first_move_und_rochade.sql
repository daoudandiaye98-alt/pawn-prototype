create table if not exists public.first_move_sessions (
...
drop policy if exists "eigene first-move-sitzung" on public.first_move_sessions;
create policy "eigene first-move-sitzung" on public.first_move_sessions
  for all to authenticated
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

drop policy if exists "admin liest first-move-sitzungen" on public.first_move_sessions;
create policy "admin liest first-move-sitzungen" on public.first_move_sessions
  for select to authenticated
  using (public.has_role(auth.uid(), 'admin'));

drop trigger if exists trg_first_move_sessions_updated on public.first_move_sessions;
