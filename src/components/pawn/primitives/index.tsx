      ...
      {(eyebrow || title || action) && (
        ...
        >
          <div className="min-w-0">
            ...
            {title && (
              <h3 className={cn("mt-1 t-display-sm", isInk && "text-white/94")}>{title}</h3>
            )}
          </div>
  ...
  return (
    <div
      className={cn(
        ...
        isInk ? "border border-white/[0.08] bg-ink" : "border border-[hsl(var(--border))] bg-card",
        className,
      )}
      title={hint}
    >
      <div className="flex items-start justify-between gap-3">
        <p className={cn("t-eyebrow", isInk && "text-white/58")}>{label}</p>
        {delta && (
          <span
            className={cn(
              "text-[0.65rem] tabular-nums",
              (trend === "up" || trend === "down") && "text-foreground",
              trend === "neutral" && (isInk ? "text-white/55" : "text-muted-foreground"),
            )}
          >
            {delta}
          </span>
      ...
      <p className={cn("mt-4 t-display-md leading-none tabular-nums", isInk && "text-white/94")}>{value}</p>
      {rationale && rationale.length > 0 && (
        <div className={cn(
          ...
          isInk
            ? "bg-gradient-to-t from-ink via-ink/95 to-transparent"
            : "bg-gradient-to-t from-card via-card/95 to-transparent",
        )}>
          <p className={cn("t-eyebrow", isInk && "text-white/58")}>Warum</p>
          <ul className={cn("mt-2 space-y-1 text-[11px] leading-snug", isInk ? "text-white/86" : "text-foreground/80")}>
            {rationale.map((r) => (
        ...
        <li key={e.id} className="flex items-start gap-3 px-5 py-3 md:px-6">
          <span
            className={cn(
              ...
              (!e.tone || e.tone === "neutral") && (isInk ? "border-white/15 text-white/78" : "border-[hsl(var(--border-strong))] text-foreground/70"),
            )}
          >
            ↳
          </span>
          <div className="min-w-0 flex-1">
            <p className={cn("t-body-sm", isInk ? "text-white/86" : "text-foreground/85")}>
              <span className={cn("font-medium", isInk ? "text-white/94" : "text-foreground")}>{e.actor}</span>{" "}
              <span className={isInk ? "text-white/66" : "text-foreground/60"}>{e.verb}</span>
              {e.object && <> <span className={isInk ? "text-white/94" : "text-foreground"}>{e.object}</span></>}
            </p>
            <p className={cn("mt-0.5 text-[10px] uppercase tracking-[0.2em]", isInk ? "text-white/45" : "text-muted-foreground")}>
              {e.when}
              {e.tag && <> · <span className={isInk ? "text-white/72" : "text-foreground/60"}>{e.tag}</span></>}
            </p>
          </div>
        </li>
      ))}
...
export function Insight({
  ...
  const sevDot =
    ...
    : "bg-foreground/40";
  return (
    <article className={cn("flex flex-col gap-3 p-5 md:p-6", isInk ? "bg-ink border border-white/[0.06]" : "bg-card border border-[hsl(var(--border))]", className)}>
      <div className="flex items-center gap-2.5">
        <span className={cn("h-2 w-2 rounded-full", sevDot)} />
        <p className={cn("t-eyebrow", isInk && "text-white/58")}>Insight</p>
        {typeof confidence === "number" && (
          <span className={cn("ml-auto text-[10px] tabular-nums uppercase tracking-[0.24em]", isInk ? "text-white/55" : "text-muted-foreground")}>
            {Math.round(confidence * 100)}% confidence
          </span>
        )}
      </div>
      <h3 className={cn("t-display-sm", isInk && "text-white/94")}>{title}</h3>
      {(cause || effect) && (
        <div className={cn("grid gap-2 text-[12.5px] leading-relaxed", isInk ? "text-white/82" : "text-foreground/80")}>
          {cause && <p><span className={cn("t-eyebrow mr-2 inline-block", isInk && "text-white/58")}>Cause</span>{cause}</p>}
          {effect && <p><span className={cn("t-eyebrow mr-2 inline-block", isInk && "text-white/58")}>Effect</span>{effect}</p>}
        </div>
      )}
...
export function Recommendation({
  ...
  return (
    <div className={cn("flex items-start gap-3 px-5 py-4 md:px-6", className)}>
      ...
      <div className="min-w-0 flex-1">
        <p className={cn("t-body-sm font-medium", isInk ? "text-white/94" : "text-foreground")}>{subject}</p>
        <p className={cn("mt-1 text-[12.5px] leading-relaxed", isInk ? "text-white/70" : "text-foreground/65")}>{rationale}</p>
  ...
  return (
    <span className={cn("inline-flex items-center gap-3", className)}>
      <span className={cn(
        ...
        isInk ? "border-white/15 bg-white/[0.03] text-white/94" : "border-[hsl(var(--border-strong))] bg-secondary text-foreground",
      )}>
        {glyph ?? name.slice(0, 1).toUpperCase()}
      </span>
      <span className="min-w-0">
        <span className={cn("block truncate t-body-sm", isInk && "text-white/94")}>{name}</span>
        {role && (
          <span className={cn("block truncate t-eyebrow", isInk && "text-white/58")}>{role}</span>
        )}
      </span>
    </span>
  );
