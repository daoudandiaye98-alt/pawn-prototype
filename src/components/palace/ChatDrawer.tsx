    ...
    <>
      ...
      >
        ...
        <header className="flex items-center justify-between border-b border-[rgba(12,12,14,.13)] px-6 py-5">
          ...
          <button onClick={onClose} className="text-[0.65rem] uppercase tracking-[0.32em] text-[#7C7972] hover:text-[#0C0C0E]">Schließen</button>
        </header>
            ...
            <div key={i} className={m.role === "assistant" ? "" : "text-right"}>
              {m.meta === "consent" ? (
                ...
                <>
                  ...
                  <p className={`mt-2 text-[0.95rem] leading-relaxed text-[#0C0C0E] ${m.role === "assistant" ? "font-serif italic" : "font-light"}`}>
                    {m.content}
                  </p>
                  {m.action && (
                    <div className="mt-4 space-y-2">
                      <button
                        ...
                        className="w-full border border-[#0C0C0E] px-4 py-3 text-left text-[0.7rem] uppercase tracking-[0.32em] text-[#0C0C0E] hover:bg-[#0C0C0E] hover:text-[#F1EEE7]"
                      >
                        {m.action.label} →
                      </button>
                    ...
                    <div className="mt-4 space-y-2">
                      {m.cards.map((c, k) => (
                        <Link key={k} to={c.href} onClick={onClose}
                          ...
                          {c.reason && <p className="mt-1 text-[0.78rem] text-[#0C0C0E]/70">{c.reason}</p>}
                        </Link>
                      ))}
                    </div>
                  )}
