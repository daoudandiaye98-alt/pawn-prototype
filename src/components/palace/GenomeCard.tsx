  ...
  return (
    <div className={cn("border-[1.5px] border-black bg-white p-6 md:p-8", props.className)}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          {props.eyebrow && <p className="editorial-eyebrow text-black/60">{props.eyebrow}</p>}
        ...
        {hasScore && (
          <p className="whitespace-nowrap font-serif text-3xl leading-none tabular-nums text-black">
            ...
            <span className="text-sm text-black/60"> / {props.score!.max ?? 100}</span>
          </p>
        )}
      </div>
        ...
        <div className="mt-6 space-y-6">
          {hasStrands && (
            <div className="space-y-3">
              {props.strandsLabel && <p className="editorial-eyebrow text-black/60">{props.strandsLabel}</p>}
              {props.strands!.map((s) => (
                <div key={s.label}>
          ...
          {hasLearned && (
            <div className="space-y-2 border-t border-black/15 pt-4">
              {props.learnedLabel && <p className="editorial-eyebrow text-black/60">{props.learnedLabel}</p>}
          ...
          {hasScore && props.score!.reasons && Object.keys(props.score!.reasons).length > 0 && (
            <ul className="space-y-1 border-t border-black/15 pt-4 text-sm text-black/80">
              {Object.entries(props.score!.reasons).map(([k, v]) => (
                <li key={k}>
                  <span className="text-black/60">{k}:</span> {v}
                </li>
              ))}
            </ul>
          )}
      ...
      {(props.signatures?.length ?? 0) > 0 && (
        <div className="mt-6 border-t border-black/15 pt-4">
          <p className="editorial-eyebrow text-black/60">Signaturen</p>
