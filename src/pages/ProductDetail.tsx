...
const ProductDetail = () => {
  ...
  function addToBag() {
    ...
    /*
     * Nicht `cart.add`, sondern `useInDenKorb`.
     *
     * Der Korb speichert nur Kennung und Größe; die Angaben zum Stück holt er
     * sich beim Anzeigen aus dem Bestand des Ladens (`core`). Dieser Bestand
     * kommt aus `src/core/seed/products.ts`, und der ist mit Absicht leer
     * (Designgesetz: keine erfundenen Daten). Ein echtes Stück aus der
     * Datenbank stand darin NIE — die Zeile wurde geschrieben, fand ihr Stück
     * beim Anzeigen nicht und fiel heraus. Wer auf „In den Korb" tippte, sah
     * eine Bestätigung und danach einen leeren Korb.
     *
     * `useInDenKorb` meldet das Stück vorher im Bestand an. Kein zweiter Korb —
     * derselbe, nur endlich gefüllt.
     */
    inDenKorb({
      ...
      name: product.name,
      preis: product.price,
      welt: (dbProduct?.world ?? null) as string | null,
      bild: dbProduct?.image_url ?? null,
      beschreibung: dbProduct?.description ?? null,
      groessen: product.sizes,
      haus: dbProduct?.designers
        ? { id: dbProduct.designers.id, slug: dbProduct.designers.slug, name: dbProduct.designers.brand_name }
        : null,
    }, size);
